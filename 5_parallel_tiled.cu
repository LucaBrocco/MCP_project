...
__global__ void kernel_forces(int N, float epsilon, float epsilon2, float *x, float *m, float *F, float *U_out)
{
    ...
    // tile memory
    __shared__ float s_x[THREADS_PER_BLOCK * 3];
    ...
    for (int tile = 0; tile < (N + blockDim.x - 1) / blockDim.x; tile++) {
        ...
        if (j_load < N) {
            s_x[threadIdx.x*3+0] = x[IDX(j_load,0)];
            s_x[threadIdx.x*3+1] = x[IDX(j_load,1)];
            s_x[threadIdx.x*3+2] = x[IDX(j_load,2)];
            s_m[threadIdx.x]     = m[j_load];
        }
        __syncthreads(); // ensure memory sync before moving on

        for (int jj = 0; jj < blockDim.x; jj++) {
            int j = tile * blockDim.x + jj;
            if (j >= N || j == i) continue;
            float dx = xi0 - s_x[jj*3+0];
            float dy = xi1 - s_x[jj*3+1];
            float dz = xi2 - s_x[jj*3+2];
            float r2    = dx*dx + dy*dy + dz*dz + epsilon2;  // manual distance: faster // 6 FLOPs (epsilon*epsilon is hoist)
            float inv_r  = rsqrtf(r2); // 1 FLOP
            float inv_r3 = inv_r * inv_r * inv_r; // 3 FLOPs
            float mimj  = mi * s_m[jj]; // 1 FLOP
    ...
    // TOTAL FLOPs 1 loop: 25
    // TOTAL: 25 * (N-1) per thread
    F[IDX(i,0)] = Fi0;
    F[IDX(i,1)] = Fi1;
    F[IDX(i,2)] = Fi2;
